...
/*  This function will do the following:
    - If MBB is an entry or exit block, set SR1 and SR2 to R2 and R12
    ...
    - If the defaults (R2/R12) are available, return true
...
PPCFrameLowering::findScratchRegister(MachineBasicBlock *MBB,
                                      ...
                                      Register *SR2) const {
  RegScavenger RS;
  Register R2 =  Subtarget.isPPC64() ? PPC::X2 : PPC::R2;
  ...
  // If MBB is an entry or exit block, use R2 and R12 as the scratch registers.
  ...
  // Note that we only return here if both R2 and R12 are available because
  ...
  if (!RS.isRegUsed(R2) && !RS.isRegUsed(R12))
    return true;
...
void PPCFrameLowering::processFunctionBeforeFrameFinalized(MachineFunction &MF,
  ...
  for (const CalleeSavedInfo &I : CSI) {
    ...
    } else if (PPC::CTRRCRegClass.contains(Reg)) {
        HasCTRSaveArea = true;
    } else {
  ...
  if (HasCTRSaveArea) {
    // Adjust the frame index of the CTR spill slot.
    for (const auto &CSInfo : CSI) {
      if (CSInfo.getReg() == PPC::CTR) {
        int FI = CSInfo.getFrameIdx();
        MFI.setObjectOffset(FI, LowerBound + MFI.getObjectOffset(FI));
        break;
      }
    ...
    LowerBound -= 4; // The CTR save area is always 4 bytes long.
  }
  ...
  for (const CalleeSavedInfo &I : CSI) {
    ...
    if (IsCRField) {
      ...
      if (!Subtarget.is32BitELFABI()) {
        ...
        // mfctr r12
        CRMIB = BuildMI(*MF, DL, TII.get(PPC::MFCTR), PPC::R12)
            .addReg(Reg, RegState::ImplicitKill);
        
        MBB.insert(MI, CRMIB);
        
        // stw r12
        MBB.insert(MI, addFrameReference(BuildMI(*MF, DL, TII.get(PPC::STW)).addReg(PPC::R12, getKillRegState(true)), I.getFrameIdx()));
    } else {
      if (I.isSpilledToReg()) {
...
bool PPCFrameLowering::restoreCalleeSavedRegisters(
  ...
  for (unsigned i = 0, e = CSI.size(); i != e; ++i) {
    ...
    if (Reg == PPC::CR2) {
      ...
      if (CTRSpilled) {
        // lwz r12
        MBB.insert(I, addFrameReference(BuildMI(*MF, DebugLoc(), TII.get(PPC::LWZ), PPC::R12), CSI[CSIIndex].getFrameIdx()));
        // mtctr r12
        MBB.insert(I, BuildMI(*MF, DebugLoc(), TII.get(PPC::MTCTR), PPC::R12).addReg(PPC::R12, getKillRegState(!(CR2Spilled || CR3Spilled || CR4Spilled))));
        CTRSpilled = false;
      }
      ...
      if (CSI[i].isSpilledToReg()) {
        ...
        // Default behavior for non-CR / non-CTR saves.
        const TargetRegisterClass *RC = TRI->getMinimalPhysRegClass(Reg);
