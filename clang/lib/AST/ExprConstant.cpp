...
public:
  ...
  bool VisitMangleExpr(const MangleExpr* E) {
      std::string Sym = E->MangleSym(Info.Ctx);
      QualType CharTy = Info.Ctx.CharTy.withConst();
      APInt Size(Info.Ctx.getTypeSize(Info.Ctx.getSizeType()), Sym.size() + 1);
      QualType ArrayTy = Info.Ctx.getConstantArrayType(CharTy, Size, nullptr, ArraySizeModifier::Normal, 0);

      StringLiteral* SL = StringLiteral::Create(Info.Ctx, Sym, StringLiteralKind::Ordinary, false, ArrayTy, E->getLocation());
      evaluateLValue(SL, Result);
      Result.addArray(Info, E, cast<ConstantArrayType>(ArrayTy));
      return true;
  }
