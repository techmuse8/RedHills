...
void ItaniumRecordLayoutBuilder::DeterminePrimaryBase(const CXXRecordDecl *RD) {
  ...
  if (RD->getNumBases() > 0) {
    const auto &I = RD->bases().begin();

    assert(!I->isVirtual() && "Oh...");
...
ItaniumRecordLayoutBuilder::LayoutBase(const BaseSubobjectInfo *Base) {
  ...
  if (Base->Class->isEmpty() &&
    ...
    if (!(EmptySubobjects->Class->isDynamicClass() && !this->PrimaryBase)) {
...
void ItaniumRecordLayoutBuilder::Layout(const CXXRecordDecl *RD) {
  ...
  // primary base, add it in at the end here.
...
static bool mustSkipTailPadding(TargetCXXABI ABI, const CXXRecordDecl *RD) {
  ...
  case TargetCXXABI::NeverUseTailPadding:
    return true;
