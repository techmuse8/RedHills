...
MangleExpr* MangleExpr::Create(const ASTContext &Ctx, SourceLocation OpLoc, SourceLocation LParen, SourceLocation RParen, NamedDecl* Decl) {
    return new (Ctx) MangleExpr(OpLoc, LParen, RParen, Ctx.getPointerType(Ctx.CharTy.withConst()), Decl);
}

std::string MangleExpr::MangleSym(ASTContext& Ctx) const {
    ASTNameGenerator mangler(Ctx);
    std::string sym = mangler.getName(Decl);
    if (sym.empty() && Decl->getIdentifier()) {
