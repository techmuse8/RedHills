...
void ItaniumVTableBuilder::ComputeThisAdjustments() {
  ...
  for (const auto &TI : VTableThunks) {
    ...
    if (MD != nullptr && MD->getParent() == MostDerivedClass)
      AddThunk(MD, Thunk);
  }
}
...
void ItaniumVTableBuilder::AddMethod(const CXXMethodDecl *MD,
  ...
  Components.push_back(VTableComponent::MakeFunctionMetadata());
  if (const CXXDestructorDecl *DD = dyn_cast<CXXDestructorDecl>(MD)) {
    assert(ReturnAdjustment.isEmpty() &&
           "Destructor can't have return adjustment!");

    // Add the complete destructor.
...
void ItaniumVTableBuilder::AddMethods(
  ...
  for (const CXXMethodDecl *MD : NewVirtualFunctions) {
    ...
    if (!ReturnAdjustment.isEmpty()) {
      VTableThunks[Components.size() + 1].Method = MD;
      VTableThunks[Components.size() + 1].ThisType = MD->getThisType().getTypePtr();
    }
