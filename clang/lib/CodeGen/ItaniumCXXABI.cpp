...
public:
  ...
  StringRef GetPureVirtualCallName() override { return "__pure_virtual_called"; }
  StringRef GetDeletedVirtualCallName() override
    { return "__deleted_virtual_called"; }
  ...
  bool HasThisReturn(GlobalDecl GD) const override {
      if (isa<CXXConstructorDecl>(GD.getDecl()) || isa<CXXDestructorDecl>(GD.getDecl()))
        return true;
    
      return CGCXXABI::HasThisReturn(GD);
  }
  
  // pretty sure this is unused but I'll add it here anyway
 ...
 private:
   bool hasAnyUnusedVirtualInlineFunction(const CXXRecordDecl *RD) const {
     ...
     for (const auto &VtableComponent : VtableLayout.vtable_components()) {
       ...
       if (Method == nullptr || !Method->getCanonicalDecl()->isInlined())
         continue;
  ...
  bool isVTableHidden(const CXXRecordDecl *RD) const {
    ...
    for (const auto &VtableComponent : VtableLayout.vtable_components()) {
      if (VtableComponent.isRTTIKind()) {
        ...
        const CXXMethodDecl *Method = VtableComponent.getFunctionDecl();
        if (Method != nullptr && Method->getVisibility() == Visibility::HiddenVisibility &&
            !Method->isDefined())
          return true;
      }
    }
...
ItaniumCXXABI::ConvertMemberPointerType(const MemberPointerType *MPT) {
  ...
  return llvm::StructType::get(CGM.Int16Ty, CGM.Int16Ty, CGM.PtrDiffTy);
}

/// In the GHS ABI, method pointers have the form:
///    struct {
///        s16 thisadj;                   // 0x0
///        s16 vtableIndex;               // 0x2
///        union {
///            void* funcptr;             // 0x4 non-virtual
///            struct {                   // virtual
///                s16 _;                 // 0x4
///                s16 vtableOffs;        // 0x6
///            }
...
CGCallee ItaniumCXXABI::EmitLoadOfMemberFunctionPointer(
  ...
  // Field 0: thisadj aka delta
  llvm::Value *Adj = Builder.CreateExtractValue(MemFnPtr, 0, "memptr.delta");
  ...
  // Field 1: vtableIndex
  llvm::Value *VTableIndex = Builder.CreateExtractValue(MemFnPtr, 1, "memptr.vtableoffset");
  
  // Field 2: funcptr_or_vtableOffset
  llvm::Value *FuncOrOffset = Builder.CreateExtractValue(MemFnPtr, 2, "memptr.func_or_index");
  
  // If the the vtable index is less than 0, the function pointer points to
  // a non-virtual function. 
  llvm::Value *IsVirtual = Builder.CreateICmpSLT(VTableIndex, llvm::ConstantInt::get(CGM.Int16Ty, 0));
  Builder.CreateCondBr(IsVirtual, FnNonVirtual, FnVirtual);
  
  // In the virtual path, the adjustment left 'This' pointing to the
  // vtable of the correct base subobject. The "function pointer" is an
  // offset within the vtable 
  CGF.EmitBlock(FnVirtual);

  // int32 at 0x4 & 0xFFFF is the same as int16 at 0x6, so let's do that.
  llvm::Value *VTableOffsetForGEP = Builder.CreateSExt(Builder.CreateAnd(FuncOrOffset, llvm::ConstantInt::get(CGM.Int32Ty, 0xFFFF)), CGM.PtrDiffTy);
  llvm::Value *VTablePtrSlotAddr = Builder.CreateInBoundsGEP(CGF.Int8Ty, This, VTableOffsetForGEP, "memptr.vtableptr_slot_addr");
  llvm::Value *VTablePtr = Builder.CreateLoad(Address(VTablePtrSlotAddr, CGF.UnqualPtrTy, CGM.getPointerAlign()), "memptr.vtableptr.load");
  
  // x * 8 + 4
  VTableIndex = Builder.CreateShl(VTableIndex, 3, "memptr.vtableindex.shl");
  VTableIndex = Builder.CreateAdd(VTableIndex, llvm::ConstantInt::get(CGM.Int16Ty, 4), "memptr.vtableindex.adjusted");
  
  // Step 3: Add the vtable pointer to the index
  llvm::Value *VirtualFn = Builder.CreateIntToPtr(Builder.CreateAdd(
      Builder.CreatePtrToInt(VTablePtr, CGM.PtrDiffTy),
      Builder.CreateSExt(VTableIndex, CGM.PtrDiffTy), "memptr.virtualfn"), CGF.UnqualPtrTy, "memptr.virtualfn.ptr");
  
  VirtualFn = Builder.CreateLoad(Address(VirtualFn, CGF.UnqualPtrTy, CharUnits::fromQuantity(4)), "memptr.virtualfn.load");
  
  CGF.EmitBranch(FnEnd);
  
  // Non-virtual path
  CGF.EmitBlock(FnNonVirtual);
  llvm::Value *NonVirtualFn = Builder.CreateIntToPtr(FuncOrOffset, CGF.UnqualPtrTy, "memptr.nonvirtualfn"); // easy
...
ItaniumCXXABI::EmitMemberPointerConversion(CodeGenFunction &CGF,
  ...
  unsigned DiagID = Diags.getCustomDiagID(clang::DiagnosticsEngine::Warning,
    "emitting untested pointer-to-member conversion (%0) code (A)");
  Diags.Report(E->getExprLoc(), DiagID) << E->getCastKindName();
  ...
  llvm::Value *srcAdj = Builder.CreateExtractValue(src, 2, "src.adj");
  llvm::Value *dstAdj;
  if (isDerivedToBase)
...
ItaniumCXXABI::EmitMemberPointerConversion(const CastExpr *E,
  ...
  clang::DiagnosticsEngine &Diags = CGM.getDiags();
  unsigned DiagID = Diags.getCustomDiagID(clang::DiagnosticsEngine::Warning,
                                          "emitting untested pointer-to-member conversion (%0) code (B)");
  Diags.Report(E->getExprLoc(), DiagID) << E->getCastKindName();
  ...
  llvm::Constant *srcAdj = src->getAggregateElement(2U);
  ...
  llvm::Constant *res = ConstantFoldInsertValueInstruction(src, dstAdj, 2);
...
ItaniumCXXABI::EmitNullMemberPointer(const MemberPointerType *MPT) {
  // GHS C++ ABI:
  //   A NULL pointer is represented as 0.
  if (MPT->isMemberDataPointer())
    return llvm::ConstantInt::get(CGM.PtrDiffTy, 0ULL, /*isSigned=*/true);

  llvm::Constant *Values[3] = { llvm::ConstantInt::get(CGM.Int16Ty, 0), llvm::ConstantInt::get(CGM.Int16Ty, 0), llvm::ConstantInt::get(CGM.PtrDiffTy, 0) };
...
llvm::Constant *ItaniumCXXABI::BuildMemberPointer(const CXXMethodDecl *MD,
  ...
  static constexpr int PTMF_delta = 0, PTMF_vtableIndex = 1, PTMF_funcPtrOrVtableOffs = 2;
  if (MD->isVirtual()) {
    // Virtual path

    uint64_t Index = CGM.getItaniumVTableContext().getMethodVTableIndex(MD) - 1;

    MemPtr[PTMF_delta] = llvm::ConstantInt::get(CGM.Int16Ty,ThisAdjustment.getQuantity());
    MemPtr[PTMF_vtableIndex] = llvm::ConstantInt::get(CGM.Int16Ty, Index);
    MemPtr[PTMF_funcPtrOrVtableOffs] = llvm::ConstantInt::get(CGM.PtrDiffTy, Context.getASTRecordLayout(MD->getParent()).getVtableFieldOffset(Context, MD->getParent()).getQuantity());
  } else {
    // Non-virtual path
    ...
    MemPtr[PTMF_delta] = llvm::ConstantInt::get(CGM.Int16Ty,ThisAdjustment.getQuantity());
    MemPtr[PTMF_vtableIndex] = llvm::ConstantInt::get(CGM.Int16Ty, -1); // -1 indicates non-virtual
    MemPtr[PTMF_funcPtrOrVtableOffs] = llvm::ConstantExpr::getPtrToInt(addr, CGM.PtrDiffTy);
  }
...
ItaniumCXXABI::EmitMemberPointerComparison(CodeGenFunction &CGF,
  ...
  // Define comparison predicates based on equality or inequality
  llvm::ICmpInst::Predicate EqPred = Inequality ? llvm::ICmpInst::ICMP_NE : llvm::ICmpInst::ICMP_EQ;
  llvm::ICmpInst::Predicate NePred = Inequality ? llvm::ICmpInst::ICMP_EQ : llvm::ICmpInst::ICMP_NE;

  // Member data pointers are easy because there's a unique null
  // value, so it just comes down to bitwise equality.
  if (MPT->isMemberDataPointer())
    return Builder.CreateICmp(EqPred, L, R);
  
  // Extract fields: { delta (index 0), vtableIndex (index 1), funcPtr (index 2) }
  llvm::Value *L_delta = Builder.CreateExtractValue(L, 0, "lhs.delta");
  llvm::Value *R_delta = Builder.CreateExtractValue(R, 0, "rhs.delta");
  llvm::Value *L_vtableIndex = Builder.CreateExtractValue(L, 1, "lhs.vtableindex");
  llvm::Value *R_vtableIndex = Builder.CreateExtractValue(R, 1, "rhs.vtableindex");
  llvm::Value *L_funcPtr = Builder.CreateExtractValue(L, 2, "lhs.funcptr");
  llvm::Value *R_funcPtr = Builder.CreateExtractValue(R, 2, "rhs.funcptr");
  
  // Compare vtableIndex
  llvm::Value *vtableIndex_cmp = Builder.CreateICmp(EqPred, L_vtableIndex, R_vtableIndex, "cmp.vtableindex");
  
  // Check vtableIndex against zero
  llvm::Value *vtableIndex_zero = Builder.CreateICmp(llvm::ICmpInst::ICMP_EQ, L_vtableIndex,
      llvm::Constant::getNullValue(L_vtableIndex->getType()), "vtableindex.zero");
  llvm::Value *vtableIndex_not_zero = Builder.CreateICmp(NePred, L_vtableIndex,
      llvm::Constant::getNullValue(L_vtableIndex->getType()), "vtableindex.notzero");

  // Compare delta and funcPtr
  llvm::Value *delta_cmp = Builder.CreateICmp(EqPred, L_delta, R_delta, "cmp.delta");
  llvm::Value *funcPtr_cmp = Builder.CreateICmp(EqPred, L_funcPtr, R_funcPtr, "cmp.funcptr");

  if (!Inequality) {
    // Equality: (vtableIndex == vtableIndex) && (vtableIndex == 0 || (delta == delta && funcPtr == funcPtr))
    llvm::Value *inner_and = Builder.CreateAnd(delta_cmp, funcPtr_cmp, "inner.and");
    llvm::Value *or_cond = Builder.CreateOr(vtableIndex_zero, inner_and, "or.cond");
    llvm::Value *result = Builder.CreateAnd(vtableIndex_cmp, or_cond, "result");
    return result;
  }

  // Inequality: (vtableIndex != vtableIndex) || (vtableIndex != 0 && (delta != delta || funcPtr != funcPtr))
  llvm::Value *delta_ne = Builder.CreateICmp(NePred, L_delta, R_delta, "cmp.delta.ne");
  llvm::Value *funcPtr_ne = Builder.CreateICmp(NePred, L_funcPtr, R_funcPtr, "cmp.funcptr.ne");
  llvm::Value *inner_or = Builder.CreateOr(delta_ne, funcPtr_ne, "inner.or");
  llvm::Value *and_cond = Builder.CreateAnd(vtableIndex_not_zero, inner_or, "and.cond");
  llvm::Value *result = Builder.CreateOr(vtableIndex_cmp, and_cond, "result");
  return result;
}
...
ItaniumCXXABI::EmitMemberPointerIsNotNull(CodeGenFunction &CGF,
  ...
  /// For member data pointers, this is just a check against 0.
  if (MPT->isMemberDataPointer()) {
    assert(MemPtr->getType() == CGM.PtrDiffTy);
    llvm::Value *Zero =
      llvm::Constant::getNullValue(MemPtr->getType());
    return Builder.CreateICmpNE(MemPtr, Zero, "memptr.tobool");
  }

  // In GHS, a member function pointer is not null if 'vtableIndex' is not 0.
  llvm::Value *VtableIndex = Builder.CreateExtractValue(MemPtr, 1, "memptr.vtableindex");

  llvm::Constant *Zero = llvm::ConstantInt::get(VtableIndex->getType(), 0);
  llvm::Value *Result = Builder.CreateICmpNE(VtableIndex, Zero, "memptr.tobool");
...
bool ItaniumCXXABI::classifyReturnType(CGFunctionInfo &FI) const {
  ...
  if (!RD->canPassInRegisters()) {
    ...
    // GHS always passes `this` before the `sret` parameter.
...
void ItaniumCXXABI::emitVirtualObjectDelete(CodeGenFunction &CGF,
                                            ...
                                            const CXXDestructorDecl *Dtor) {
  EmitVirtualDestructorCall(CGF, Dtor, Dtor_Complete, Ptr, DE);
}
...
ItaniumCXXABI::buildStructorSignature(GlobalDecl GD,
  ...
  // Check if we need to add the secret dtor flag
  if (isa<CXXDestructorDecl>(GD.getDecl())) {
...
void ItaniumCXXABI::addImplicitStructorParams(CodeGenFunction &CGF,
  ...
  if (isa<CXXDestructorDecl>(MD)) {
    auto *DtorFlag = ImplicitParamDecl::Create(
      Context, nullptr, MD->getLocation(), &Context.Idents.get("dtorflag"), Context.IntTy, ImplicitParamKind::Other
    );
    Params.insert(Params.begin() + 1, DtorFlag);
    getStructorImplicitParamDecl(CGF) = DtorFlag;
  }
}
...
llvm::Value *ItaniumCXXABI::getCXXDestructorImplicitParam(
    ...
    bool ForVirtualBase, bool Delegating) {
  return getStructorImplicitParamValue(CGF);
}
...
void ItaniumCXXABI::EmitDestructorCall(CodeGenFunction &CGF,
  ...
  QualType DtorTy = getContext().IntTy;
  
  const CXXMethodDecl *CurrentDtor =
      dyn_cast_or_null<CXXDestructorDecl>(CGF.CurCodeDecl);

  if (CurrentDtor && CurrentDtor->getParent() != DD->getParent() && !Delegating) {
    // This is a base destructor call. Force the implicit "delete flag" parameter to a constant 0.
    DtorFlag = llvm::ConstantInt::get(CGF.IntTy, 0);
  } else {
    DtorFlag =
      getCXXDestructorImplicitParam(CGF, DD, Type, ForVirtualBase, Delegating);
    if (DtorFlag == nullptr) {
        // Fallback for safety, in case no parameter value is found.
        DtorFlag = llvm::ConstantInt::get(CGF.IntTy, 0);
    }
  }
  ...
  CGF.EmitCXXDestructorCall(GD, Callee, CGF.getAsNaturalPointerTo(This, ThisTy),
                            ThisTy, DtorFlag, DtorTy, nullptr);
}
...
void ItaniumCXXABI::emitVTableDefinitions(CodeGenVTables &CGVT,
                                          const CXXRecordDecl *RD) {
  llvm::GlobalVariable *VTable = getAddrOfVTable(RD, getContext().getASTRecordLayout(RD).getVtableFieldOffset(getContext(), RD));
...
ItaniumCXXABI::getVTableAddressPoint(BaseSubobject Base,
  ...
  llvm::GlobalValue *VTable = getAddrOfVTable(VTableClass, getContext().getASTRecordLayout(VTableClass).getNonVirtualSize());
...
llvm::Value *ItaniumCXXABI::EmitVirtualDestructorCall(
  ...
  llvm::Value *DtorFlag = nullptr;
  if (CE != nullptr) { // Explicit dtor call
    DtorFlag = llvm::ConstantInt::get(CGM.IntTy, 2);
  } else if (D != nullptr) { // Automatic dtor call by delete
    DtorFlag = llvm::ConstantInt::get(CGM.IntTy, 3);
  }
  
  assert(DtorFlag != nullptr);

  CGF.EmitCXXDestructorCall(GD, Callee, This.emitRawPointer(CGF), ThisTy,
                            DtorFlag, getContext().IntTy, nullptr);
  return nullptr;
}
