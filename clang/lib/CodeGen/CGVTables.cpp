...
void CodeGenFunction::EmitCallAndReturnForThunk(llvm::FunctionCallee Callee,
  ...
  if (isa<CXXDestructorDecl>(MD)) {
    ImplicitParamDecl *DtorFlagDecl = CGM.getCXXABI().getStructorImplicitParamDecl(*this);
    assert(DtorFlagDecl && "Could not find dtorflag implicit parameter decl");
    assert(DtorFlagDecl->getType() == getContext().IntTy && "dtorflag is not an int");

    llvm::Value *DtorFlagVal = Builder.CreateLoad(GetAddrOfLocalVar(DtorFlagDecl));

    CallArgs.add(RValue::get(DtorFlagVal), getContext().IntTy);
  }
...
void CodeGenVTables::addVTableComponent(ConstantArrayBuilder &builder,
  ...
  case VTableComponent::CK_FunctionPointer:
    if (component.getFunctionDecl() == nullptr) {
