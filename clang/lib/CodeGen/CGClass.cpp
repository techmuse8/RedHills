...
void CodeGenFunction::EmitConstructorBody(FunctionArgList &Args) {
  ...
  llvm::Value* ThisNull = Builder.CreateICmpEQ(This, llvm::ConstantPointerNull::get(
        llvm::cast<llvm::PointerType>(This->getType())), "ctor.this.isnull");
  
  llvm::BasicBlock* AllocBB = createBasicBlock("ctor.alloc");
  llvm::BasicBlock* ContinueBB = createBasicBlock("ctor.continue");
  
  Builder.CreateCondBr(ThisNull, AllocBB, ContinueBB);
  
  EmitBlock(AllocBB);
  llvm::Value* AllocatedPtr = nullptr;
  { // Allocate and set `this`
      FunctionDecl* allocator = getContext().getAllocatorFunction(Ctor->getParent()->getCanonicalDecl());
      
      assert(allocator != nullptr && "Looked up null allocator");
      ...
      llvm::Value* callee = CGM.GetAddrOfFunction(allocator);
      llvm::FunctionType* calleeTy = llvm::FunctionType::get(llvm::PointerType::getUnqual(CGM.getLLVMContext()), {SizeTy}, false);
      
      AllocatedPtr = Builder.CreateCall(calleeTy, callee, {size});
  }
  
  EmitBlock(ContinueBB);
  
  // Merge alloc and non-alloc paths
  llvm::PHINode* thisPHI = Builder.CreatePHI(This->getType(), 2, "this_phi");
  thisPHI->addIncoming(This, EntryBB);
  thisPHI->addIncoming(AllocatedPtr, AllocBB);
  
  CXXThisValue = thisPHI;
  CXXABIThisValue = thisPHI;
  
  This = LoadCXXThis();
  ThisNull = Builder.CreateICmpEQ(This, llvm::ConstantPointerNull::get(
        llvm::cast<llvm::PointerType>(This->getType())), "ctor.this.isstillnull");
  
  llvm::BasicBlock* AllocFailBB = createBasicBlock("ctor.alloc.fail");
  llvm::BasicBlock* AllocSuccessBB = createBasicBlock("ctor.alloc.success");
  
  Builder.CreateCondBr(ThisNull, AllocFailBB, AllocSuccessBB);
  
  EmitBlock(AllocSuccessBB);
  ...
  // --- AllocFailBB ---
  EmitBlock(AllocFailBB);

  llvm::AllocaInst* retvalAlloca = nullptr;
  llvm::Function* F = Builder.GetInsertBlock()->getParent();

  if (F && !F->isDeclaration() && !F->getEntryBlock().empty()) {
      llvm::BasicBlock& EntryB = F->getEntryBlock();
      for (llvm::Instruction& I : EntryB) {
          if (I.hasName() && I.getName() == "retval") {
              if (llvm::AllocaInst* AI = llvm::dyn_cast<llvm::AllocaInst>(&I)) {
                  if (AI->getAllocatedType() == F->getReturnType()) {
                      retvalAlloca = AI;
                      break;
                  }
              }
  ...
  if (retvalAlloca) {
      clang::CharUnits storeAlignment;
      llvm::MaybeAlign alignFromAlloca = retvalAlloca->getAlign();
  
      if (alignFromAlloca.has_value()) {
          storeAlignment = clang::CharUnits::fromQuantity(alignFromAlloca.value().value());
      } else {
          storeAlignment = CharUnits::One() * 4;
      }
  
      clang::CodeGen::Address destAddress(
          retvalAlloca,                        
          retvalAlloca->getAllocatedType(),
          storeAlignment                     
      );

      Builder.CreateStore(LoadCXXThis(), destAddress);
  
  } else {
      CGM.getDiags().Report(Ctor->getLocation(),
          CGM.getDiags().getCustomDiagID(clang::DiagnosticsEngine::Error,
          "Constructor codegen: Could not find 'retval' AllocaInst. "
          "The returned 'this' pointer may be incorrect if allocation occurred."));
  }
}
...
namespace {
  ...
  void EmitConditionalDtorDeleteCall(CodeGenFunction &CGF,
    ...
    llvm::Value *ShouldCallDelete
      = CGF.Builder.CreateIsNull(CGF.Builder.CreateAnd(ShouldDeleteCondition, llvm::ConstantInt::get(ShouldDeleteCondition->getType(), 1)));
...
void CodeGenFunction::InitializeVTablePointer(const VPtr &Vptr) {
  ...
  if (CGM.getCXXABI().isVirtualOffsetNeededForVTableField(*this, Vptr)) {
    ...
    // The vtable field exist at the end of the class, so place it after the tail field of the first polymorphic base
    NonVirtualOffset += CGM.getContext().getASTRecordLayout(Vptr.VTableClass).getVtableFieldOffset(CGM.getContext(), Vptr.Base.getBase());
  }
...
llvm::Value *CodeGenFunction::GetVTablePtr(Address This,
  ...
  CharUnits VTableFieldOffset = CGM.getContext().getASTRecordLayout(RD).getVtableFieldOffset(CGM.getContext(), RD);

  llvm::Value* ptr = This.emitRawPointer(*this); //? getPointer instead? Idk
  ptr = Builder.CreateInBoundsGEP(Int8Ty, ptr, llvm::ConstantInt::get(CGM.PtrDiffTy, VTableFieldOffset.getQuantity(), true), "add.ptr");
  
  CharUnits alignment = This.getAlignment();
  alignment = alignment.alignmentAtOffset(VTableFieldOffset);
  
  Address VTablePtrSrc = Address(ptr, Int8Ty, alignment).withElementType(VTableTy);
