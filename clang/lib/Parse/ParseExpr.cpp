...
ExprResult Parser::ParseCastExpression(CastParseKind ParseKind,
  ...
  case tok::kw___builtin_sycl_unique_stable_name:
    ...
    break;
  case tok::kw___builtin_mangle:
    Res = ParseMangleExpression();
    break;
...
/// Parse a __builtin_mangle expression.
ExprResult Parser::ParseMangleExpression() {
  assert(Tok.is(tok::kw___builtin_mangle) && "Not __builtin_mangle");
  ...
  if (T.expectAndConsume(diag::err_expected_lparen_after, "__builtin_mangle")) {
      err = true;
  }
  
  CXXScopeSpec SS;
  if (!err && ParseOptionalCXXScopeSpecifier(SS, nullptr, false, false)) {
      err = true;
  }
  
  UnqualifiedId Name;
  if (!err && ParseUnqualifiedId(SS, nullptr, false, false, true, true, false, nullptr, Name)) {
      err = true;
  }
  
  ParsedType Signature;
  if (Tok.is(tok::comma)) {
      ConsumeToken();
  ...
  return Actions.ActOnMangleExpr(OpLoc, T.getOpenLocation(), T.getCloseLocation(), SS, Name, Signature);
}
