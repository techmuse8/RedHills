...
void ASTWriter::WriteSpecialDeclRecords(Sema &SemaRef) {
  ...
  auto& AllocatorFuncs = Context.AllocatorFunctions;
  if (!AllocatorFuncs.empty()) {
    // Sort for deterministic output
    SmallVector<std::pair<const CXXRecordDecl *, const FunctionDecl *>> SortedMap(AllocatorFuncs.begin(), AllocatorFuncs.end());
    llvm::sort(SortedMap, [this](const auto &LHS, const auto &RHS) {
      return GetDeclRef(LHS.first) < GetDeclRef(RHS.first);
    });
    
    // Serialize ptrs into data
    RecordData Record;
    for (const auto &Pair : SortedMap) {
      AddDeclRef(Pair.first, Record);
      AddDeclRef(Pair.second, Record);
    }
    
    // Write it
    Stream.EmitRecord(ALLOCATOR_FUNCS, Record);
  }
}
