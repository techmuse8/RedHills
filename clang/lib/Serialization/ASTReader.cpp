...
llvm::Error ASTReader::ReadASTBlock(ModuleFile &F,
  ...
  while (true) {
    ...
    case ALLOCATOR_FUNCS: {
        if (Record.empty()) break;
        
        // Iterate over each entry
        unsigned Idx = 0;
        while (Idx < Record.size()) {
            // Get the raw DeclIDs from the record
            GlobalDeclID RecordID = ReadDeclID(F, Record, Idx);
            GlobalDeclID FuncID = ReadDeclID(F, Record, Idx);
            
            // Deserialize to pointers
            CXXRecordDecl* CXXRec = cast_or_null<CXXRecordDecl>(GetDecl(RecordID));
            FunctionDecl* Func = cast_or_null<FunctionDecl>(GetDecl(FuncID));
            // Push it to the context
            if (CXXRec && Func) {
                getContext().AllocatorFunctions[CXXRec] = Func;
            }
        }
