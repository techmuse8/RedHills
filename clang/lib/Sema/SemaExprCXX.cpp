...
bool Sema::FindAllocationFunctions(SourceLocation StartLoc, SourceRange Range,
  ...
  if (PassAlignment) {
    ...
    assert(getLangOpts().AlignedAllocation &&
           "PassAlignment should only be true if aligned allocation is enabled");
...
ExprResult Sema::ActOnMangleExpr(SourceLocation OpLoc, SourceLocation LParen, SourceLocation RParen, CXXScopeSpec& SS, UnqualifiedId& Name, ParsedType Signature) {
    DeclarationNameInfo NameInfo = GetNameFromUnqualifiedId(Name);
    LookupResult R(*this, NameInfo, LookupOrdinaryName);
    
    if (NameInfo.getName().getNameKind() == DeclarationName::CXXConstructorName) {
        if (CXXRecordDecl* RD = SS.getScopeRep()->getAsRecordDecl()) {
            R.clear();
            ...
            CXXConstructorDecl* CD = nullptr;
            for (CXXConstructorDecl* Ctor: RD->ctors()) {
                if (!first) {
                    overloaded = true;
                    
                    // if we have an overload we need a sig
                    if (!Signature) {
                        Diag(NameInfo.getLoc(), diag::err_ovl_ambiguous_call) << NameInfo.getName();
                        return ExprError();
                    }
                    
                    // if we have an overload and a sig then check it and set the ptr
                    QualType TargetType = GetTypeFromParser(Signature);
                    if (Context.hasSameType(Ctor->getType(), TargetType)) {
                        CD = Ctor;
                        foundOverload = true;
                        break;
                    }
            ...
            if (overloaded) {
                if (foundOverload) {
                    ...
                    Diag(NameInfo.getLoc(), diag::err_undeclared_label_use) << NameInfo.getName();
                }
            } else if (CD) {
                R.addDecl(CD);
            } else {
        ...
        LookupParsedName(R, getCurScope(), &SS, QualType());
    }
    ...
    if (R.isOverloadedResult()) {
        if (!Signature) {
            ...
            for (NamedDecl* D : R) {
                ...
                if (FD && Context.hasSameType(FD->getType(), TargetType)) {
                    ND = FD;
                    break;
                }
            }
        ...
        Diag(NameInfo.getLoc(), diag::err_ambiguous_reference) << NameInfo.getName();
    }
    ...
    return MangleExpr::Create(getASTContext(), OpLoc, LParen, RParen, ND);
}
