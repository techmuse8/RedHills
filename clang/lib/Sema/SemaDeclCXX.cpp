...
void Sema::CheckCompletedCXXClass(Scope *S, CXXRecordDecl *Record) {
  ...
  { // Locate the allocator to use in the ctor
      FunctionDecl* NewFunction = nullptr;
      FunctionDecl* DeleteFunction = nullptr;
      bool PassAlignment = false;
      
      assert (getLangOpts().AlignedAllocation == false && "C++17 aligned allocation is unsupported. Use -fno-aligned-allocation");
      
      bool Success = FindAllocationFunctions(
          Record->getLocation(), SourceRange(),
          AllocationFunctionScope::AFS_Both, AllocationFunctionScope::AFS_Both,
          QualType(Record->getTypeForDecl(), 0), false,
          PassAlignment, {}, NewFunction, DeleteFunction,
          true
      );
      
      assert(Success == 0 && "Failed to find allocation functions");

      assert(NewFunction != nullptr &&
          "Allocation function should be found for a record type");

      getASTContext().setAllocatorFunction(Record->getCanonicalDecl(), NewFunction);
  }
