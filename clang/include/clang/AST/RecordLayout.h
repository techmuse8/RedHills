...
public:
  ...
  /// getVtableFieldOffset - Get the offset of the vtable field of the given class
  /// which is the final member of the first dynamic class in the inheritance hierarchy.
  CharUnits getVtableFieldOffset(const ASTContext& context, const CXXRecordDecl* D) const {
    assert(CXXInfo && "Record layout does not have C++ specific info!");
    
    const CXXRecordDecl* vtableIntroducer = context.baseForVTableAuthentication(D);
    const CharUnits size = context.getASTRecordLayout(vtableIntroducer).getNonVirtualSize();
    const CharUnits vtableOffset = size - (CharUnits::One() * 4);
