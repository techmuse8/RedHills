...
class ASTContext : public RefCountedBase<ASTContext> {
  ...
  /// Mapping from CXXRecordDecls to their corresponding allocator FunctionDecls
  llvm::DenseMap<const CXXRecordDecl *, FunctionDecl *> AllocatorFunctions;
...
public:
  ...
  void setAllocatorFunction(const CXXRecordDecl* RD, FunctionDecl* F) {
      AllocatorFunctions[RD] = F;
  }
  
  FunctionDecl* getAllocatorFunction(const CXXRecordDecl* RD) {
    auto I = AllocatorFunctions.find(RD);
    if (I != AllocatorFunctions.end())
      return I->second;
    return nullptr;
  }
