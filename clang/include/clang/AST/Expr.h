...
class MangleExpr final : public Expr {
    friend class ASTStmtReader;
    SourceLocation OpLoc, LParen, RParen;
    NamedDecl* Decl;
...
public:
    MangleExpr(SourceLocation OpLoc, SourceLocation LParen, SourceLocation RParen, QualType ResultTy, NamedDecl* Decl)
        : Expr(MangleExprClass, ResultTy, VK_PRValue, OK_Ordinary) , OpLoc(OpLoc), LParen(LParen), RParen(RParen), Decl(Decl) {
            setDependence(ExprDependence::None);
        }
    ...
    void setDecl(NamedDecl* D) { Decl = D; }
    
    std::string MangleSym(ASTContext& Context) const;
    
    static bool classof(const Stmt *T) {
      return T->getStmtClass() == MangleExprClass;
    }
    ...
    static MangleExpr* Create(const ASTContext &Ctx, SourceLocation OpLoc, SourceLocation LParen, SourceLocation RParen, NamedDecl* Decl);
};
